...
public class AStar extends DijkstraAlgorithm {
	...
	protected ShortestPathSolution doRun() {
        ...
        labels[start.getId()].setCost(0);
        tas.insert(labels[start.getId()]);
        
        notifyOriginProcessed(start);
        
        while(!tas.isEmpty() && labels[data.getDestination().getId()].isMarque() == false) {
        	Label label_courrant = tas.deleteMin();
